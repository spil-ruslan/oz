...
class RedHatCDYumGuest(RedHatCDGuest):
    ...
    def _internal_customize(self, libvirt_xml, action):
        ...
        try:
            ...
            if action == "gen_only":
                self.log.debug('Using "safe" snapshot based ICICLE generation for gen_only request')
    ...
    def _create_snapshot(self, modified_xml):
        # Since we are not modifying here we can use a snapshot to
        # avoid any unwanted side effects from booting the guest.  The
        # mechanics are slightly confusing.  When creating the
        # snapshot a new disk file is generated by libvirt.  This new
        # disk file is a qcow2 writeable snapshot regardless of
        # whether the original disk is raw or qcow2.  It contains any
        # changes made after the snapshot is taken.  The original disk
        # file is thus preserved unchanged.
        ...
        # Force the snapshot to be external - if the bootdisk is qcow2
        # already, libvirt may try to use internal snapshots imcleod
        # note - testing with libvirt-0.10.2-18.el6.x86_64 this was
        # not the case - even a qcow2 base file results in a distinct
        # external snapshot - however I do not believe this is
        # guaranteed
        # FIXME: Find out if this will ever be the case
        ...
        # Now that we have taken a snapshot, libvirt_dom's disk is
        # backed by a writeable qcow2 snapshot.  We want to discard
        # this when we are done but libvirt does not yet have a way to
        # delete the file for us.  So, we determine its name here and
        # unlink it later.
        ...
        # Now that we have taken a snapshot we can also do the
        # collect_setup mods without changing our original disk.  We
        # pass in the current XML description of the domain, which
        # points to our writeable snapshot disk
